...
def main():
    ...
    # 在sidebar增加买卖记录栏目
    pages = ["📊 股票分析", "⚙️ 配置管理", "💾 缓存管理", "💰 Token统计", "📈 历史记录", "🔧 系统状态", "💹 买卖记录"]
    ...
    if page == "⚙️ 配置管理":
        ...
        try:
            ...
            cursor = conn.cursor()
            # 增加股票代码搜索框
            stock_code_search = st.text_input("股票代码筛选（可留空）", value="")
            # 筛选条件
            ...
            params = []
            where_clauses = []
            if stock_code_search.strip():
                where_clauses.append("stock_symbol = %s")
                params.append(stock_code_search.strip())
            if selected_advice != '全部':
                if selected_advice == '无建议':
                    where_clauses.append("(final_advice IS NULL OR final_advice = '')")
                else:
                    where_clauses.append("final_advice = %s")
    ...
    if session_id:
        ...
        db_manager.create_tables_if_not_exist()
        # --- 新增/编辑表单 ---
        edit_mode = st.session_state.get("edit_trade_id", None)
        # 确保所有变量有默认值，避免未定义或None
        default_symbol = default_type = default_time = default_shares = default_price = default_total = default_reason = default_market = default_company = ""
        if edit_mode:
            record = db_manager.get_trade_record(edit_mode)
            if record:
                default_symbol = record[1] or ""
                default_type = record[4] or ""
                if isinstance(record[5], str):
                    default_time = record[5] or ""
                else:
                    default_time = record[5].strftime("%Y-%m-%d %H:%M:%S")
                default_shares = record[6] or ""
                default_price = float(record[7]) if record[7] is not None else ""
                default_total = float(record[8]) if record[8] is not None else ""
                default_reason = record[9] or ""
                default_market = record[3] or ""
                default_company = record[2] or ""
        else:
            default_type = "买入" # 新增记录默认买入
        ...
        with st.form("trade_form", clear_on_submit=not edit_mode):
            col1, col2, col3 = st.columns(3)
            with col1:
                market_type = st.selectbox("市场类型", ["A股", "美股"], index=0)
                stock_symbol = st.text_input("股票代码", value=default_symbol or "")
                # 自动补全公司名称，使用prepare_stock_data
                company_name = ""
                if stock_symbol.strip():
                    try:
                        from tradingagents.utils.stock_validator import prepare_stock_data
                        result = prepare_stock_data(stock_symbol.strip(), market_type)
                        if result.is_valid:
                            company_name = result.stock_name
                        else:
                            st.error(f"股票代码 '{stock_symbol}' 不存在或无效。")
                    except Exception as e:
                        company_name = ""
                    st.info(f"公司名称：{company_name if company_name else '未找到'}")
            with col2:
                trade_type = st.selectbox("买卖类型", ["买入", "卖出"], index=0 if default_type=="买入" else 1)
            with col3:
                trade_time = st.text_input("买卖时间(YYYY-MM-DD HH:MM:SS)", value=default_time or datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
            shares = st.number_input("股数", min_value=1, value=int(default_shares) if str(default_shares).isdigit() else 100)
            price_per_share = st.number_input("每股价格", min_value=0.01, value=float(default_price) if default_price else 10.0, format="%.2f")
            total_amount = shares * price_per_share
            st.info(f"总金额：{total_amount:.2f} 元 (自动计算)")
            reason = st.text_area("买卖原因", value=default_reason or "")
            submitted = st.form_submit_button("保存")
            if submitted:
                # 保存时强校验公司名称
                if not company_name:
                    st.error("股票代码不存在或无效，无法保存！")
                    # 只提示，不中断，后续内容继续展示
                else:
                    if edit_mode:
                        db_manager.update_trade_record(edit_mode, stock_symbol, company_name, market_type, trade_type, trade_time, shares, price_per_share, total_amount, reason)
                        st.success("修改成功！")
                        st.session_state["edit_trade_id"] = None
                        st.rerun()
                    else:
                        db_manager.insert_trade_record(stock_symbol, company_name, market_type, trade_type, trade_time, shares, price_per_share, total_amount, reason)
                        st.success("新增成功！")
                        st.rerun()
        # --- 买卖记录表格 ---
        st.subheader("买卖记录列表")
        trade_rows = db_manager.get_trade_records()
        if trade_rows:
            import pandas as pd
            trade_df = pd.DataFrame(trade_rows, columns=["ID", "股票代码", "公司名称", "市场", "类型", "时间", "股数", "单价", "总金额", "原因", "创建时间"])
            st.dataframe(trade_df.drop("ID", axis=1), use_container_width=True, hide_index=True)
            st.markdown("---")
            for row in trade_rows:
                cols = st.columns([6,1,1,1])
                with cols[0]:
                    st.markdown(f"<span style='font-size:15px;'>【{row[4]}】{row[1]}({row[2]}) {row[3]} {row[5]}股 x {row[6]}元 = {row[7]}元</span>", unsafe_allow_html=True)
                with cols[1]:
                    if st.button("详情", key=f"detail_{row[0]}"):
                        st.session_state["show_trade_detail"] = row[0]
                with cols[2]:
                    if st.button("编辑", key=f"edit_{row[0]}"):
                        st.session_state["edit_trade_id"] = row[0]
                        st.rerun()
                with cols[3]:
                    if st.button("删除", key=f"delete_{row[0]}"):
                        db_manager.delete_trade_record(row[0])
                        st.success("删除成功！")
                        st.rerun()
        else:
            st.info("暂无买卖记录")
        # --- 详情页 ---
        trade_id = st.session_state.get("show_trade_detail")
        if trade_id:
            trade_row = db_manager.get_trade_record(trade_id)
            if trade_row:
                st.header(f"买卖详情 (ID: {trade_id})")
                st.write(f"**股票代码**: {trade_row[1]}")
                st.write(f"**公司名称**: {trade_row[2]}")
                st.write(f"**市场**: {trade_row[3]}")
                st.write(f"**类型**: {trade_row[4]}")
                st.write(f"**时间**: {trade_row[5]}")
                st.write(f"**股数**: {trade_row[6]}")
                st.write(f"**单价**: {trade_row[7]}")
                st.write(f"**总金额**: {trade_row[8]}")
                st.write(f"**原因**: {trade_row[9]}")
                st.write(f"**创建时间**: {trade_row[10]}")
                st.subheader("该股票所有历史分析主表信息：")
                session_rows = db_manager.get_mysql_conn().cursor()
                session_rows.execute("SELECT id, stock_symbol, market_type, analysis_date, created_at, final_advice FROM report_sessions WHERE stock_symbol=%s ORDER BY created_at DESC", (trade_row[1],))
                sessions = session_rows.fetchall()
                session_rows.close()
                if sessions:
                    for s in sessions:
                        st.markdown(f"<span style='font-size:15px;'>分析ID:{s[0]} 市场:{s[2]} 日期:{s[3]} 建议:{s[5]} 创建:{s[4]}</span> <a href='/record_detail?session_id={s[0]}' target='_self' style='color:#1a73e8;font-weight:bold;'>主表详情</a>", unsafe_allow_html=True)
                else:
                    st.info("该股票暂无历史分析记录")
                if st.button("返回列表", key="back_to_list"):
                    st.session_state["show_trade_detail"] = None
                    st.rerun()
            st.stop()
    else:
        # 默认显示股票分析页面
